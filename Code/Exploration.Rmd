...
Though the phylogeny tree clearly showed the relationship between species. The species are too much to explore, we then try to make a treemap instead (<https://r-graph-gallery.com/236-custom-your-treemap.html>).
...
We can reject the null hypothesis (P = \<2.2e-16) i.e. variances are not equal.
...
There is significant difference in hatching temperature among families (F = 184.5, P \<2e-16). The result corresponds with the box plot we made above. This variation of temperature among families can be due to the genetic variation or the population conditions.
