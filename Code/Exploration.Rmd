...
First, we need to filter the data because we have too many species. Let's only focus on Family level data.
```{r stats}
...
## view location of data on the map
```{r location of data}
library(tidyverse)
library(sf)
library(leaflet)
...
# plot
ggplot() +
  geom_sf(data = Cleaned_Data_sf,)

mapview(Cleaned_Data_sf)
```
